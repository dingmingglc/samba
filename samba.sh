...
share() { local share="$1" path="$2" browsable="${3:-yes}" ro="${4:-yes}" \
                ...
                writelist="${8:-""}" comment="${9:-""}" readlist="${10:-""}"            file=/etc/samba/smb.conf
    sed -i "/\\[$share\\]/,/^\$/d" $file
    echo "[$share]" >>$file
    echo "   path = $path" >>$file
...
user() { local name="$1" passwd="$2" id="${3:-""}" group="${4:-""}" \
    ...
    grep -q "^$name:" /etc/passwd ||
        adduser -S -D -H -h /tmp -s /sbin/nologin ${group:+-G $group} ${id:+-u $id} -g 'Customzie Samba User' "$name"
...
### enableforceuser: disable user force user and force group
# Arguments:
#   none)
# Return: result
enableforceuser() { local file=/etc/samba/smb.conf 
    sed -i '/force group/d;/force user/d;/force create mode/d;/force directory mode/d;' $file 
}
...
while getopts ":hc:G:g:i:nprs:Su:Ww:I:fF:" opt; do
    case "$opt" in
        h) usage ;;
        c) charmap "$OPTARG" ;;
